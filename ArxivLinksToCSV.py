...
def fetch_paper_details(urls):
    ...
    for url in urls:
        ...
        abstract = page.find('blockquote', class_='abstract').text.replace('Abstract:', '').strip()
...
def save_to_csv(papers_data, filename):
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow(['Title', 'Link', 'Date', 'Authors', 'Abstract'])
        for paper in papers_data:
            writer.writerow([paper['title'], paper['paper_link'], paper['submit_date'], paper['authors'], paper['abstract']])
