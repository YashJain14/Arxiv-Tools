...
def fetch_arxiv_data(url):
    ...
    while True:
        ...
        if not entries:
            break  # No more papers found, exit the loop
        ...
        # Check for next set of papers
        skip_value = int(re.search(r'skip=(\d+)', url).group(1)) + 2000
        url = re.sub(r'skip=\d+', f'skip={skip_value}', url)
    return data
